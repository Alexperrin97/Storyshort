...
export default function App() {
  ...
  const buildEpPrompt = (epMeta, s) => {
    ...
    return [
      "Tu es scénariste expert en vertical drama (TikTok/Reels/Shorts).",
      "Série: " + bible.title,
      "Synopsis série: " + bible.synopsis,
      "Personnages: " + chars,
      "Lieux: " + lieux,
      dnaBlock,
      "",
      "Écris l\'ÉPISODE " + (epMeta.index + 1) + ' : "' + epMeta.title + '"',
      "Synopsis: " + epMeta.synopsis,
      "Hook: " + epMeta.hook,
      "Personnages de cet épisode: " + (epMeta.mainCharacters || []).join(", "),
      "Lieu principal: " + epMeta.location,
      "Durée cible: " + s.dur,
      "Fin de l\'épisode: " + epMeta.cliffhanger,
      "",
      "FORMAT OBLIGATOIRE (respecte exactement ce modèle) :",
      "ÉPISODE X — TITRE",
      "INT. NOM DU LIEU – MOMENT",
      "",
      "NOM PERSONNAGE",
      "Réplique du personnage.",
      "",
      "NOM PERSONNAGE",
      "(indication de jeu entre parenthèses si nécessaire)",
      "Réplique.",
      "",
      "RÈGLES ABSOLUES :",
      "- Noms de personnages en MAJUSCULES seuls sur leur ligne, avant chaque réplique",
      "- Indications de scène : INT./EXT. LIEU – MOMENT",
      "- Transitions entre scènes : une ligne simple type 'elipse maison' ou 'ellipse soir'",
      "- JAMAIS de directions caméra (pas de 'zoom', 'cut to', 'gros plan', etc.)",
      "- JAMAIS de 'à suivre' ou 'fin de l\'épisode' à la fin",
      "- JAMAIS de numérotation de plans",
      "- Indications de jeu courtes entre parenthèses SOUS le nom du personnage si besoin",
      "- Langue: " + s.langue,
      "- Dialogues percutants, courts, tension montante",
      "- Hook dès la première réplique",
      "",
      "Écris UNIQUEMENT le script de l\'épisode, rien d\'autre."
    ].join("\n");
  };
  ...
  const [exportTab, setExportTab] = useState("script"); // script | props | costumes
  const [exportData, setExportData] = useState(null); // { props, costumes }
  const [exportLoading, setExportLoading] = useState(false);

  const buildFullScript = () => {
    if (!bible || !Object.keys(scripts).length) return "";
    const header = bible.title.toUpperCase() + "\n" + (bible.tagline || "") + "\n\n";
    const charBlock = "PERSONNAGES\n" + (bible.characters || []).map(c =>
      c.name + " – " + c.role + (c.stakes ? " | Enjeu : " + c.stakes : "")
    ).join("\n") + "\n\n";
    const epScripts = bible.episodes.map((ep, i) =>
      (scripts[i] || "[Script non généré]")
    ).join("\n\n" + "─".repeat(60) + "\n\n");
    return header + charBlock + epScripts;
  };

  const generateExportData = async () => {
    setExportLoading(true);
    const allScripts = buildFullScript();
    try {
      const raw = await callAPI(null, [{
        role: "user",
        content: [
          "Analyse ces scripts et retourne UNIQUEMENT ce JSON valide (sans markdown) :",
          '{"props":[{"nom":"...","description":"...","episodes":[1],"jours":["J1"]}],"costumes":[{"personnage":"...","tenues":[{"description":"...","episodes":[1],"jours":["J1"],"moment":"Jour/Nuit/Matin/Soir"}]}]}',
          "",
          "REGLES IMPORTANTES :",
          "- props : chaque accessoire/objet physique mentionné ou utilisé (contrat, téléphone, verre, dossier, enveloppe, robe, stylo...)",
          "- costumes : pour chaque personnage principal, liste chaque tenue distincte (ex: robe noire, costume sombre, pyjama...)",
          "- Pour chaque item, identifie le ou les JOURS (J1, J2, J3...) en te basant sur les marqueurs temporels dans les scripts (INT. SALON – MATIN J1, etc.)",
          "- Si le jour n'est pas explicitement mentionné, déduis-le du contexte narratif (début de série = J1, etc.)",
          "- episodes = tableau des numéros d'épisodes (commence à 1)",
          "- jours = tableau des jours identifiés (ex: ['J1', 'J2'])",
          "- moment = moment de la journée (Matin, Après-midi, Soir, Nuit)",
          "",
          "SCRIPTS :",
          allScripts.slice(0, 14000)
        ].join("\n")
      }]);
      const parsed = parseJSON(raw);
      if (parsed) setExportData(parsed);
      else throw new Error("Parsing export JSON échoué");
  ...
  const downloadPDF = async (type) => {
    ...
    const accent = [185, 0, 255];
    const pink = [251, 0, 243];
    const white = [255, 255, 255];
    const grey = [180, 160, 210];
    const dimgrey = [100, 80, 130];
    const dark = [15, 10, 20];
    const surface = [24, 18, 32];
    const border = [50, 30, 80];

    // helpers
    const setFont = (size, style="normal", color=white) => {
      doc.setFontSize(size); doc.setFont("helvetica", style); doc.setTextColor(...color);
    };
    const fillRect = (x, fy, w, h, color) => { doc.setFillColor(...color); doc.rect(x, fy, w, h, "F"); };
    const newPage = () => { doc.addPage(); fillRect(0, 0, W, H, dark); y = mt; };
    const checkY = (need=12) => { if (y + need > H - 16) newPage(); };
    const wrapText = (text, maxW, fontSize, style="normal", color=white) => {
      setFont(fontSize, style, color);
      return doc.splitTextToSize(text || "", maxW);
    };
    const drawLines = (lines, x, indent, lineH, fontSize, style="normal", color=white) => {
      setFont(fontSize, style, color);
      for (const line of lines) { checkY(lineH); doc.text(line, x + indent, y); y += lineH; }
    };

    // Cover page bg
    fillRect(0, 0, W, H, dark);
    ...
    if (type === "script") {
      ...
      doc.roundedRect(ml, 28, usableW, 42, 4, 4, "F");
      doc.setFillColor(...accent);
      doc.circle(ml + 22, 49, 12, "F");
      setFont(22, "bold", white); doc.text("▶", ml + 17.5, 52);
      setFont(26, "bold"); 
      doc.setTextColor(...accent); doc.text("Story", ml + 40, 46);
      doc.setTextColor(...pink); doc.text("short", ml + 40 + doc.getTextWidth("Story"), 46);
      setFont(9, "normal", dimgrey); doc.text("SCRIPT", ml + 40, 54);

      // Title block
      y = 88;
      const titleLines = wrapText((bible?.title || "").toUpperCase(), usableW, 32, "bold", white);
      for (const line of titleLines) { doc.setFontSize(32); doc.setFont("helvetica","bold"); doc.setTextColor(...white); doc.text(line, W/2, y, {align:"center"}); y += 12; }
      y += 4;
      setFont(13, "normal", grey);
      const tagLines = wrapText(bible?.tagline || "", usableW - 20, 13);
      for (const line of tagLines) { doc.text(line, W/2, y, {align:"center"}); y += 7; }

      // Divider
      y += 8;
      doc.setDrawColor(...accent); doc.setLineWidth(0.5); doc.line(ml + 20, y, W - mr - 20, y); y += 10;

      // Meta
      setFont(10, "normal", dimgrey);
      const ep = bible?.episodes?.length || 0;
      doc.text(ep + " ÉPISODE" + (ep>1?"S":""), W/2, y, {align:"center"}); y += 6;
      doc.text(new Date().toLocaleDateString("fr-FR", {year:"numeric",month:"long"}), W/2, y, {align:"center"});

      // Characters
      y = H - 80;
      setFont(8, "bold", [accent[0], accent[1], accent[2]]);
      doc.text("PERSONNAGES", ml, y); y += 6;
      doc.setDrawColor(...border); doc.setLineWidth(0.3); doc.line(ml, y, W-mr, y); y += 5;
      for (const c of (bible?.characters || []).slice(0, 6)) {
        setFont(8, "bold", white); doc.text(c.name, ml, y);
        setFont(8, "normal", grey); doc.text("  " + (c.role || ""), ml + doc.getTextWidth(c.name), y);
        y += 5;
      }
      ...
      for (let i = 0; i < bible.episodes.length; i++) {
        ...
        for (const line of lines) {
          ...
          // Episode header
          if (/^(ÉPISODE|EPISODE)/i.test(t)) {
            checkY(14);
            doc.setFillColor(...surface);
            doc.rect(ml, y - 5, usableW, 10, "F");
            doc.setDrawColor(...accent); doc.setLineWidth(1);
            doc.line(ml, y - 5, ml, y + 5);
            setFont(10, "bold", accent);
            doc.text(t, ml + 4, y + 1);
            y += 10;
          }
          // Scene heading INT/EXT
          else if (/^(INT\.|EXT\.)/.test(t)) {
            checkY(10);
            y += 3;
            setFont(8, "bold", grey);
            doc.text(t, ml, y); y += 7;
          }
          // Stage direction (parens or brackets)
          else if (/^[\(\[]/.test(t)) {
            checkY(7);
            setFont(8, "italic", dimgrey);
            const wrapped = doc.splitTextToSize(t, usableW - 20);
            for (const wl of wrapped) { doc.text(wl, ml + 10, y); y += 5; }
          }
          // Character name (ALL CAPS short line)
          else if (/^[A-ZÀÂÉÈÊËÎÏÔÙÛÜÇ][A-ZÀÂÉÈÊËÎÏÔÙÛÜÇ\s\-]{1,30}$/.test(t) && t.length < 35) {
            checkY(9);
            y += 3;
            setFont(9, "bold", pink);
            doc.text(t, ml + 20, y); y += 6;
          }
          // Ellipsis/transition
          else if (/^(elipse|ellipse|cut|fondu)/i.test(t)) {
            checkY(7);
            setFont(7, "italic", dimgrey);
            doc.text(t, W/2, y, {align:"center"}); y += 6;
          }
          // Regular dialogue
          else {
            checkY(6);
            ...
            for (const wl of wrapped) { checkY(5); doc.text(wl, ml + 5, y); y += 5; }
          }
        }
      }
      doc.save((bible?.title || "script").replace(/[^a-zA-Z0-9]/g,"_") + "_script.pdf");
    }
      ...
      setFont(9, "bold", accent); doc.text("STORYSHORT SCRIPT", ml, 18);
      y = 30;
      setFont(28, "bold", white); doc.text("ACCESSOIRES", ml, y); y += 10;
      setFont(11, "normal", grey);
      const subLines = doc.splitTextToSize((bible?.title || "") + " — Récap production", usableW);
      for(const l of subLines) { doc.text(l, ml, y); y += 7; }
      doc.setDrawColor(...accent); doc.setLineWidth(0.5); doc.line(ml, y+2, W-mr, y+2); y += 12;

      const props = exportData?.props || [];
      setFont(9, "normal", dimgrey);
      doc.text(props.length + " accessoire" + (props.length>1?"s":"") + " identifié" + (props.length>1?"s":""), ml, y); y += 12;

      for (const p of props) {
        checkY(28);
        doc.setFillColor(...surface); doc.setDrawColor(...border); doc.setLineWidth(0.3);
        doc.roundedRect(ml, y, usableW, 22, 2, 2, "FD");
        // Icon circle
        doc.setFillColor(...accent);
        doc.circle(ml + 8, y + 11, 5, "F");
        setFont(7, "bold", dark); doc.text("🎭", ml + 5.5, y + 12.5);
        // Content
        setFont(10, "bold", white); doc.text(p.nom || "", ml + 17, y + 8);
        const descLines = doc.splitTextToSize(p.description || "", usableW - 20);
        setFont(8, "normal", grey);
        for (let di = 0; di < Math.min(descLines.length, 2); di++) {
          doc.text(descLines[di], ml + 17, y + 13 + di * 4.5);
        }
        // Tags
        let tx = ml + 17;
        const ty = y + 20;
        for (const ep of (p.episodes || []).slice(0, 8)) {
          const label = "Ép." + ep;
          const tw = doc.getTextWidth(label) + 4;
          doc.setFillColor(...accent); doc.setFillColor(accent[0], accent[1], accent[2], 0.2);
          doc.setFillColor(30, 15, 45);
          doc.roundedRect(tx, ty-3, tw, 4.5, 1, 1, "F");
          doc.setDrawColor(...accent); doc.setLineWidth(0.2);
          doc.roundedRect(tx, ty-3, tw, 4.5, 1, 1, "S");
          setFont(7, "bold", accent); doc.text(label, tx + 2, ty + 0.5);
          tx += tw + 2;
        }
        for (const j of (p.jours || []).slice(0, 4)) {
          const label = j;
          const tw = doc.getTextWidth(label) + 4;
          doc.setFillColor(30, 40, 15);
          doc.roundedRect(tx, ty-3, tw, 4.5, 1, 1, "F");
          doc.setDrawColor(80, 200, 100); doc.setLineWidth(0.2);
          doc.roundedRect(tx, ty-3, tw, 4.5, 1, 1, "S");
          setFont(7, "bold", [80,200,100]); doc.text(label, tx + 2, ty + 0.5);
          tx += tw + 2;
        }
        y += 26;
      }
      doc.save((bible?.title || "serie").replace(/[^a-zA-Z0-9]/g,"_") + "_accessoires.pdf");
    }

    else if (type === "costumes") {
      // Cover
      fillRect(0, 0, W, H, dark);
      doc.setFillColor(...pink); doc.rect(0, 0, W, 3, "F");
      setFont(9, "bold", pink); doc.text("STORYSHORT SCRIPT", ml, 18);
      y = 30;
      setFont(28, "bold", white); doc.text("TENUES", ml, y); y += 10;
      setFont(11, "normal", grey);
      const subL = doc.splitTextToSize((bible?.title || "") + " — Costumes par personnage & jour", usableW);
      for(const l of subL) { doc.text(l, ml, y); y += 7; }
      doc.setDrawColor(...pink); doc.setLineWidth(0.5); doc.line(ml, y+2, W-mr, y+2); y += 12;

      const costumes = exportData?.costumes || [];
      for (const char of costumes) {
        checkY(18);
        // Character header
        doc.setFillColor(...surface);
        doc.roundedRect(ml, y, usableW, 12, 2, 2, "F");
        doc.setFillColor(...pink); doc.circle(ml + 6, y + 6, 4, "F");
        setFont(11, "bold", white); doc.text(char.personnage || "", ml + 14, y + 7.5);
        const nb = (char.tenues || []).length;
        setFont(8, "normal", grey); doc.text(nb + " tenue" + (nb>1?"s":""), W-mr-2, y + 7.5, {align:"right"});
        y += 16;

        for (let ti = 0; ti < (char.tenues || []).length; ti++) {
          const t = char.tenues[ti];
          checkY(24);
          doc.setFillColor(18, 12, 28);
          doc.setDrawColor(...border); doc.setLineWidth(0.3);
          doc.roundedRect(ml + 4, y, usableW - 4, 20, 2, 2, "FD");
          // Tenue number badge
          doc.setFillColor(...pink);
          doc.roundedRect(ml + 7, y + 4, 12, 12, 1.5, 1.5, "F");
          setFont(9, "bold", dark); doc.text("T" + (ti+1), ml + 8.5, y + 11.5);
          // Description
          setFont(9, "bold", white); doc.text(t.description || "", ml + 23, y + 8);
          // Moment
          if (t.moment) { setFont(7, "normal", grey); doc.text(t.moment, ml + 23, y + 13); }
          // Episode + jour tags
          let tx = ml + 23;
          const tagY = y + 18;
          for (const ep of (t.episodes || []).slice(0, 6)) {
            const label = "Ép." + ep;
            const tw = doc.getTextWidth(label) + 4;
            doc.setFillColor(25, 10, 40);
            doc.roundedRect(tx, tagY - 3, tw, 4.5, 1, 1, "F");
            doc.setDrawColor(...accent); doc.setLineWidth(0.2);
            doc.roundedRect(tx, tagY - 3, tw, 4.5, 1, 1, "S");
            setFont(7, "bold", accent); doc.text(label, tx + 2, tagY + 0.5);
            tx += tw + 2;
          }
          for (const j of (t.jours || []).slice(0, 4)) {
            const tw = doc.getTextWidth(j) + 4;
            doc.setFillColor(10, 30, 20);
            doc.roundedRect(tx, tagY - 3, tw, 4.5, 1, 1, "F");
            doc.setDrawColor(80, 200, 100); doc.setLineWidth(0.2);
            doc.roundedRect(tx, tagY - 3, tw, 4.5, 1, 1, "S");
            setFont(7, "bold", [80,200,100]); doc.text(j, tx + 2, tagY + 0.5);
            tx += tw + 2;
          }
          y += 24;
        }
      ...
      doc.save((bible?.title || "serie").replace(/[^a-zA-Z0-9]/g,"_") + "_tenues.pdf");
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center", flexShrink: 0 }}>
          ...
          {step === "board" && bible && (
            <>
              ...
              <Btn small variant="primary" onClick={() => { setExportPanel(true); if (!exportData) generateExportData(); }}>
                ⬇ Exporter
              </Btn>
            </>
          )}
      ...
      {exportPanel && step === "board" && (
        <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, width: "100%", maxWidth: 760, maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {/* Export header */}
            <div style={{ padding: "18px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>⬇ Export — {bible?.title}</div>
                <div style={{ fontSize: 12, color: C.textDim, marginTop: 2 }}>{Object.keys(scripts).length} épisodes · {totalWords.toLocaleString()} mots</div>
              </div>
              <button onClick={() => setExportPanel(false)} style={{ background: "none", border: "none", color: C.textDim, cursor: "pointer", fontSize: 22, lineHeight: 1 }}>×</button>
            </div>

            {/* Tabs */}
            <div style={{ display: "flex", gap: 4, padding: "12px 24px 0", borderBottom: `1px solid ${C.border}` }}>
              {[
                { id: "script", icon: "📄", label: "Script complet" },
                { id: "props", icon: "🎭", label: "Accessoires" },
                { id: "costumes", icon: "👗", label: "Tenues par personnage" },
              ].map(t => (
                <button key={t.id} onClick={() => setExportTab(t.id)} style={{
                  padding: "8px 16px", background: exportTab === t.id ? C.accentSoft : "transparent",
                  border: `1px solid ${exportTab === t.id ? C.accentGlow : "transparent"}`,
                  borderBottom: "none", borderRadius: "8px 8px 0 0", color: exportTab === t.id ? C.accent : C.textDim,
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
              ...
              {exportTab === "script" && (
                <div>
                  <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
                    <Btn variant="primary" onClick={() => downloadPDF("script")}>
                      ⬇ Télécharger le script PDF
                    </Btn>
                    <Btn variant={copied === "full" ? "success" : "ghost"} small onClick={copyFullScript}>
                      {copied === "full" ? "✓ Copié !" : "⎘ Copier en texte brut"}
                    </Btn>
                  </div>
                  <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 20px", marginBottom: 10 }}>
                    <div style={{ fontSize: 11, color: C.textDim, textTransform: "uppercase", letterSpacing: ".1em", fontWeight: 700, marginBottom: 10 }}>Contenu du PDF</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <span style={{ fontSize: 13 }}>📄</span>
                        <span style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>Page de couverture</span>
                        <span style={{ fontSize: 11, color: C.textDim }}>Titre, tagline, liste des personnages</span>
                      </div>
                      {bible?.episodes?.map((ep, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          <span style={{ fontSize: 11, color: C.accent, fontWeight: 700, minWidth: 50 }}>ÉP. {i+1}</span>
                          <span style={{ fontSize: 12, color: C.textMid }}>{ep.title}</span>
                          {scripts[i] ? <span style={{ fontSize: 10, color: C.success }}>✓</span> : <span style={{ fontSize: 10, color: C.textDim }}>—</span>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {exportTab === "props" && (
                <div>
                  {exportLoading ? (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 0", gap: 12, color: C.textDim }}>
                      <Spinner /> <span>Analyse des accessoires et des jours...</span>
                    </div>
                  ) : exportData?.props ? (
                    <div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <span style={{ fontSize: 13, color: C.textDim }}>
                          {exportData.props.length} accessoire{exportData.props.length > 1 ? "s" : ""} identifié{exportData.props.length > 1 ? "s" : ""}
                        </span>
                        <Btn variant="primary" small onClick={() => downloadPDF("props")}>⬇ Télécharger PDF</Btn>
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {exportData.props.map((p, i) => (
                          <div key={i} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 14, alignItems: "flex-start" }}>
                            <div style={{ width: 32, height: 32, background: C.accentSoft, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>🎭</div>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontWeight: 700, color: C.text, fontSize: 14, marginBottom: 3 }}>{p.nom}</div>
                              <div style={{ fontSize: 12, color: C.textMid, marginBottom: 6 }}>{p.description}</div>
                              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                                {(p.episodes || []).map(e => (
                                  <span key={e} style={{ fontSize: 10, color: C.accent, background: C.accentSoft, border: `1px solid ${C.accentGlow}`, borderRadius: 4, padding: "2px 7px", fontWeight: 600 }}>ÉP. {e}</span>
                                ))}
                              </div>
                            </div>
                          </div>
                    ...
                    <div style={{ textAlign: "center", padding: "40px 0", color: C.textDim }}>
                      <Btn onClick={generateExportData} variant="outline">Analyser les accessoires</Btn>
                    </div>
                  )}
                </div>
              )}
              ...
              {exportTab === "costumes" && (
                <div>
                  {exportLoading ? (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 0", gap: 12, color: C.textDim }}>
                      <Spinner /> <span>Analyse des tenues...</span>
                    </div>
                  ) : exportData?.costumes ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 13, color: C.textDim }}>
                          {exportData.costumes.length} personnage{exportData.costumes.length > 1 ? "s" : ""} · tenues par jour identifiées
                        </span>
                        <Btn variant="primary" small onClick={() => downloadPDF("costumes")}>⬇ Télécharger PDF</Btn>
                      </div>
                      {exportData.costumes.map((c, i) => (
                        <div key={i} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
                          <div style={{ padding: "12px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 28, height: 28, background: `linear-gradient(135deg,${C.accent},${C.pink})`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>👤</div>
                            <div style={{ fontWeight: 800, color: C.text, fontSize: 15 }}>{c.personnage}</div>
                            <Tag color={C.textMid}>{(c.tenues || []).length} tenue{(c.tenues || []).length > 1 ? "s" : ""}</Tag>
                          </div>
                          <div style={{ padding: "12px 18px", display: "flex", flexDirection: "column", gap: 8 }}>
                            {(c.tenues || []).map((t, j) => (
                              <div key={j} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "10px 14px", background: C.surface, borderRadius: 8 }}>
                                <div style={{ minWidth: 28, height: 28, background: `linear-gradient(135deg,${C.accent},${C.pink})`, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff", fontWeight: 800 }}>T{j+1}</div>
                                <div style={{ flex: 1 }}>
                                  <div style={{ fontSize: 13, color: C.text, fontWeight: 600, marginBottom: 2 }}>{t.description}</div>
                                  {t.moment && <div style={{ fontSize: 11, color: C.textDim, marginBottom: 5 }}>⏰ {t.moment}</div>}
                                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                                    {(t.episodes || []).map(e => (
                                      <span key={"ep"+e} style={{ fontSize: 10, color: "#50a8ff", background: "#50a8ff18", border: "1px solid #50a8ff30", borderRadius: 4, padding: "2px 7px", fontWeight: 600 }}>Ép. {e}</span>
                                    ))}
                                    {(t.jours || []).map(j2 => (
                                      <span key={"j"+j2} style={{ fontSize: 10, color: C.success, background: C.successBg, border: `1px solid ${C.success}30`, borderRadius: 4, padding: "2px 7px", fontWeight: 600 }}>{j2}</span>
                                    ))}
                                  </div>
                                </div>
                              </div>
                    ...
                    <div style={{ textAlign: "center", padding: "40px 0", color: C.textDim }}>
                      <Btn onClick={generateExportData} variant="outline">Analyser les tenues</Btn>
                    </div>
                  )}
                </div>
              )}
